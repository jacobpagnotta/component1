...
data.Event = {'R01'}
data.Facility = {'Lab', 'E&R', 'XYZ Hospital', 'ABC Clinic'}
...
local function ranStatus()
   local R = ranChoose(data.Status)
   return R
end 
...
local function ranPD1(PD1)
   PD1[3][1][1] = ranChoose(data.Facility) 
   PD1:S()
end
...
local function ranOBR(OBR)
   ...
   OBR[4][1] = r[1]
   OBR[4][2] = r[2]
   OBR[32][1][1] = math.random(999999)
   OBR[32][1][2] = ranLastName()
   OBR[32][1][3] = ranFirstName()
   OBR:S()
end
...
local HELP_DEF={
   ...
   Desc = "Generates sample HL7 ORU messages from the seed data (hard-coded into the module)",
   Usage = "ran.RandomMessage()",
   ParameterTable=false,
   Parameters = none,
   Returns = {{Desc='A generated HL7 ORU message <u>string</u>'}},
